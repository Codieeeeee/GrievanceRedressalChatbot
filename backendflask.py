    ...
    try:
        ...
        if image_data:
            ...
            if location_data.get('error'):
                return jsonify({
                    "success": False,
                    "message": "The provided image does not contain valid GPS data. Please upload an image with GPS metadata."
                }), 400
            if location_data.get('address'):
                # If GPS data is successfully extracted, use it instead of form address
                final_address = location_data['address']

        # Debugging: Print final address
        ...
        cursor.execute("""
            INSERT INTO complaints (ticket_number, user_id, department_id, description, location_data, image_path)
            VALUES (%s, %s, %s, %s, %s, %s)
...
def track_complaint():
    try:
        ...
        cursor.execute("""
            SELECT c.ticket_number, c.description, c.status, c.created_at, c.location_data,
...
def get_all_complaints():
    ...
    query = """
        SELECT c.id, c.ticket_number, c.description, c.status, 
               c.created_at, c.updated_at, c.location_data,  -- Include location_data
               d.name as department, u.name as user_name, u.email as user_email
        FROM complaints c
...
def get_complaint_details(complaint_id):
    try:
        ...
        # Fetch complaint details, including location_data (address)
        cursor.execute("""
            SELECT c.id, c.ticket_number, c.description, c.status, 
                   c.created_at, c.updated_at, c.image_path,
                   c.location_data,  -- Use location_data instead of address
